...
-- ============================================================================
-- Escopo:
-- - Migration candidate revisavel para persistencia auditavel pre-obra.
-- - Sem FK para orcamento_itens.
...
-- - pgcrypto/gen_random_uuid() esta disponivel.
-- - As 9 tabelas deste candidate nao existem no banco real.
--
-- Decisao de vocabulario:
-- - Status tecnicos de pipeline permanecem em ingles.
-- - agreement_band permanece em ingles por ser metrica tecnica.
-- - Status operacional de divergencia foi harmonizado para portugues.
-- - severity permanece em portugues: baixa/media/alta/critica.
-- ============================================================================
...
COMMENT ON COLUMN public.orc_reader_runs.opportunity_id IS
  'Ancora obrigatoria de auditoria/RLS do Orçamentista IA.';
COMMENT ON COLUMN public.orc_reader_runs.orcamento_id IS
  'Nullable nesta fase; torna-se obrigatorio somente antes de consolidacao oficial futura.';
...
COMMENT ON TABLE public.orc_reader_outputs IS
  'Output bruto e normalizado do Reader. raw_output_json e protegido por trigger de imutabilidade.';
COMMENT ON COLUMN public.orc_reader_outputs.raw_output_json IS
  'Payload bruto do Reader. Imutavel apos INSERT; correcoes devem gerar novo output.';
...
COMMENT ON COLUMN public.orc_reader_verifier_comparisons.agreement_band IS
  'Metrica tecnica mantida em ingles: low/medium/high.';
...
CREATE TABLE IF NOT EXISTS public.orc_reader_verifier_divergences (
  ...
  status text NOT NULL DEFAULT 'aberta' CHECK (
    status IN ('aberta', 'aceita', 'descartada', 'resolvida')
  ),
  created_at timestamptz NOT NULL DEFAULT now(),
...
COMMENT ON COLUMN public.orc_reader_verifier_divergences.status IS
  'Status operacional harmonizado em portugues no candidate 4A.5.';
...
CREATE TABLE IF NOT EXISTS public.orc_context_snapshots (
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT orc_context_snapshot_has_source_ck CHECK (
    source_id IS NOT NULL
    OR NULLIF(trim(COALESCE(source_ref, '')), '') IS NOT NULL
    OR reader_run_id IS NOT NULL
    OR reader_output_id IS NOT NULL
    OR verifier_run_id IS NOT NULL
    OR comparison_id IS NOT NULL
    OR hitl_issue_id IS NOT NULL
    OR source_refs_json <> '{}'::jsonb
  )
);
...
-- ============================================================================
-- INDICES MINIMOS E HARDENING 4A.3
-- ============================================================================
...
-- ============================================================================
-- TRIGGERS DE PROTECAO / IMUTABILIDADE
-- ============================================================================
-- Estas funcoes nao escrevem em outras tabelas e nao tocam orcamento_itens.
...
BEGIN
  IF OLD.raw_output_json IS DISTINCT FROM NEW.raw_output_json THEN
    RAISE EXCEPTION 'raw_output_json is immutable after insert';
  END IF;
...
CREATE TRIGGER trg_orc_reader_outputs_prevent_raw_update
  BEFORE UPDATE ON public.orc_reader_outputs
  FOR EACH ROW
  EXECUTE FUNCTION public.fn_orc_reader_outputs_prevent_raw_update();
...
-- ROW LEVEL SECURITY
-- ============================================================================
-- RLS fica habilitado nas 9 tabelas.
-- Policies definitivas ficam pendentes ate decisao de auth/tenant/company_id.
-- Sem policies, o acesso direto fica bloqueado por padrao para roles nao privilegiados.
-- Nao criar policies abertas do tipo USING (true) nestas tabelas.

ALTER TABLE public.orc_reader_runs ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.orc_reader_outputs ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.orc_reader_safety_evaluations ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.orc_verifier_runs ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.orc_reader_verifier_comparisons ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.orc_reader_verifier_divergences ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.orc_hitl_issues ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.orc_hitl_decisions ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.orc_context_snapshots ENABLE ROW LEVEL SECURITY;

-- Placeholder documental para fase posterior:
-- CREATE POLICY ... ON public.orc_reader_runs
--   FOR SELECT
--   TO authenticated
